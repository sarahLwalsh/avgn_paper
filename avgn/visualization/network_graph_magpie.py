...
def plot_network_graph(
    ...
    # generate graph
    graph = compute_graph(
        transition_matrix, min_connections=0.001, column_names=true_label
    )
    ...
    nx.draw_networkx_edges(graph, pos, ax=ax, edge_color=rgba_cols, width=10, connectionstyle="arc3, rad=0.2")
    ...
    ax.scatter(pos_locs[:, 0], pos_locs[:, 1], color=pos_colors, s=3000, zorder=1500)
    
    B=nx.to_edgelist(graph)
    print(B)
